 ...
 */
pub enum ConditionType<TokenConditions, ElementToken> {
    Single(ElementToken),
    Multiple(TokenConditions),
}

// type CT<T> = ConditionType<T>;

pub enum Instruction<TokenConditions, ElementToken, ResultToken>
where
    &'static [(Option<(TokenConditions, bool)>, ResultToken, Address)]: 'static,
{
    Match((ConditionType<TokenConditions, ElementToken>, bool)),
    Goto((ConditionType<TokenConditions, ElementToken>, bool), Address),
    // IsOnly(ElementToken),
    // NotOnly(ElementToken),
    // Is(TokenConditions), // Is("") 可被用于故意错误退出
    // Not(TokenConditions), // Not("") 可被用于跳转后的占位匹配
    // IsGoto(TokenConditions, Address), // 匹配成功跳转，匹配不成功继续前进，除跳转到-1地址外，匹配过程不消耗字符，即会让本字符继续沿着下一条指令匹配，跳转到-1地址代表成功结束
    // NotGoto(TokenConditions, Address), // NotGoto("") 可用于构造loop
    Call(&'static [(Option<(TokenConditions, bool)>, ResultToken, Address)], usize), // 调用其他匹配规则，推荐使用调用前提条件以提高效率
    ...
    DebugIsGoto(TokenConditions, Address, &'static str), // 匹配成功跳转，匹配不成功继续前进，除跳转到-1地址外，匹配过程不消耗字符，即会让本字符继续沿着下一条指令匹配，跳转到-1地址代表成功结束
    DebugNotGoto(TokenConditions, Address, &'static str), // NotGoto("") 可用于构造loop
    DebugCall(&'static [(ResultToken, Address)], usize, &'static str),
...
pub const MATCH_TERMINAL: A = A::Absolute(-1);
pub const NEXT: A = A::Relative(1);
...
where
    ...
    &'static [Instruction<TokenConditions, ElementToken, ResultToken>]: From<ResultToken> + 'static,
{
    pub token: Vec<Token<ElementToken, ResultToken, L>>,
    pub rule: ResultToken,
    rule_instructions: &'static [Instruction<TokenConditions, ElementToken, ResultToken>],
    address: i32,
    pub location: L,
...
where
    ...
    L: Clone + TokenLocation<ElementToken>,
    &'static [Instruction<TokenConditions, ElementToken, ResultToken>]: From<ResultToken>,
{
    ...
    fn match_instruction(&mut self, c: &ElementToken) -> Option<Result<Option<(Token<ElementToken, ResultToken, L>, bool)>, (Self, String)>> {
        loop {
            ...
            match match &self.rule_instructions.get(match <i32 as TryInto<usize>>::try_into(self.address) {
                ...
                Instruction::Match((condition, is_or_not)) => {
                    if match condition {
                        ConditionType::Single(condition) => (condition == c) == *is_or_not,
                        ConditionType::Multiple(conditions) => conditions.contains(c) == *is_or_not,
                    } {
                ...
                Instruction::Goto((condition, is_or_not), address) => {
                    if match condition {
                        ConditionType::Single(condition) => (condition == c) == *is_or_not,
                        ConditionType::Multiple(conditions) => conditions.contains(c) == *is_or_not,
                    } {
                        let target_address = match address {
                            Address::Absolute(i) => *i,
                            Address::Relative(i) => self.address + i,
                        };
                        if target_address == -1 {
                            self.token.push(Token::Element(c.clone(), self.location.clone()));
                                ...
                                match rules.get(i) {
                                    Some((precondition, rule, address)) => {
                                        ...
                                        let target_address = match address {
                                            ...
                                            Address::Relative(a) => self.address + a,
                                        };
                ...
                Instruction::ErrEnd(result_token) => {
